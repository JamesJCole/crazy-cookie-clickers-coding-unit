...
- Understanding the make code interface.
- Create a **variable** to track your cookies
- Add a cookie **sprite** to the screen

Let's go!

## Step 1 — Learning the blocks

First, let's make the game look great with a cool background.

Find the ``||scene:set background Image to||`` block in the **Scene** drawer and drag it into the ``||loops:on start||`` block.
...
If you want to change the look of your cookie, click on the 'grey' circle with the cookie inside!
...
Set **x** to **45** and **y** to **70** — that's the centre of the screen!
